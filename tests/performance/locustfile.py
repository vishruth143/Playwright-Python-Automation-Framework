...
Run     : locust -f tests/performance/locustfile.py --headless -u 10 -r 2 -t 60s
          (config-driven: set REGION env var for different environments)
...
class JsonPlaceholderUser(HttpUser):
    ...
    host = _CONFIG.get("base_url", "https://jsonplaceholder.typicode.com")
    wait_time = between(
    ...
    def get_user_todos(self):
        """GET /users/{id}/todos - fetch todos for a random user."""
    ...
    def create_post(self):
        ...
        self.client.post("/posts", json=self._CREATE_PAYLOAD, name="POST /posts")
    @task(2)
    ...
    def patch_post(self):
        ...
        post_id = random.choice(self._POST_IDS)
        self.client.patch(f"/posts/{post_id}", json=self._PATCH_PAYLOAD, name="PATCH /posts/{id}")
    @task(1)
...
def on_locust_init(environment, **kwargs):
    """Print the active config at startup."""
...
def on_request(request_type, name, response_time, response_length,
    ...
    if exception:
        print(f"[FAIL] {request_type} {name} | time={response_time:.0f}ms | error={exception}")
    elif response and response.status_code >= 400:
        print(f"[FAIL] {request_type} {name} | status={response.status_code} | time={response_time:.0f}ms")
